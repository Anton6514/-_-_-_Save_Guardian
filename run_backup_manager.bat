...
:: Скрипт для запуска основного менеджера бэкапа на PowerShell в скрытом окне.
...
:: Запускаем PowerShell в скрытом окне (-WindowStyle Hidden), указывая ему выполнить наш скрипт.
:: -ExecutionPolicy Bypass позволяет обойти политику выполнения скриптов (если она мешает).
:: -File указывает путь к скрипту.
powershell -WindowStyle Hidden -ExecutionPolicy Bypass -File "%~dp0%PS_SCRIPT%"
...
:: Убираем паузу и сообщение, чтобы .bat файл завершился тихо
